...
public class GroupV2ConflictMergerTest {
  ...
  public void merge_alwaysPreferRemote_exceptProfileSharingIsEitherOr() {
    SignalGroupV2Record remote = new SignalGroupV2Record.Builder(TestHelpers.byteArray(1), groupKey(100))
                                                          .setBlocked(false)
                                                          .setProfileSharingEnabled(false)
                                                          .setArchived(false)
                                                          .build();
    SignalGroupV2Record local  = new SignalGroupV2Record.Builder(TestHelpers.byteArray(2), groupKey(100))
                                                          .setBlocked(true)
  ...
  private static GroupMasterKey groupKey(int value) {
    try {
      return new GroupMasterKey(TestHelpers.byteArray(value, 32));
