...
import br.eti.meta.storage.StorageSyncHelper.KeyDifferenceResult;
import br.eti.meta.storage.StorageSyncHelper.MergeResult;
...
public final class StorageSyncHelperTest {
  ...
  public void resolveConflict_noOverlap() {
    ...
    Assert.assertEquals(TestHelpers.setOf(remote1), result.getLocalContactInserts());
    assertTrue(result.getLocalContactUpdates().isEmpty());
    Assert.assertEquals(TestHelpers.setOf(SignalStorageRecord.forContact(local1)), result.getRemoteInserts());
  ...
  public void resolveConflict_contact_deleteSelfContact() {
    ...
    assertTrue(result.getLocalContactUpdates().isEmpty());
    Assert.assertEquals(TestHelpers.setOf(record(local1)), result.getRemoteInserts());
    assertTrue(result.getRemoteUpdates().isEmpty());
    Assert.assertEquals(TestHelpers.setOf(remote1), result.getRemoteDeletes());
  }
  ...
  public void resolveConflict_contact_sameAsRemote() {
    ...
    assertTrue(result.getLocalContactInserts().isEmpty());
    Assert.assertEquals(TestHelpers.setOf(update(local1, expectedMerge)), result.getLocalContactUpdates());
  ...
  public void resolveConflict_group_v1_sameAsRemote() {
    ...
    assertTrue(result.getLocalContactInserts().isEmpty());
    Assert.assertEquals(TestHelpers.setOf(update(local1, expectedMerge)), result.getLocalGroupV1Updates());
    assertTrue(result.getRemoteInserts().isEmpty());
    assertTrue(result.getRemoteUpdates().isEmpty());
    assertTrue(result.getRemoteDeletes().isEmpty());
  }
  ...
  public void resolveConflict_group_v2_sameAsRemote() {
    ...
    assertTrue(result.getLocalContactInserts().isEmpty());
    Assert.assertEquals(TestHelpers.setOf(update(local1, expectedMerge)), result.getLocalGroupV2Updates());
    assertTrue(result.getRemoteInserts().isEmpty());
    assertTrue(result.getRemoteUpdates().isEmpty());
    assertTrue(result.getRemoteDeletes().isEmpty());
  }
  ...
  public void resolveConflict_contact_sameAsLocal() {
    ...
    assertTrue(result.getLocalContactUpdates().isEmpty());
    assertTrue(result.getRemoteInserts().isEmpty());
    Assert.assertEquals(TestHelpers.setOf(recordUpdate(remote1, expectedMerge)), result.getRemoteUpdates());
    assertTrue(result.getRemoteDeletes().isEmpty());
  }
  ...
  public void resolveConflict_unknowns() {
    ...
    MergeResult result = StorageSyncHelper.resolveConflict(TestHelpers.setOf(remote1, remote2, account), TestHelpers.setOf(local1, local2, account));

    assertTrue(result.getLocalContactInserts().isEmpty());
    assertTrue(result.getLocalContactUpdates().isEmpty());
    Assert.assertEquals(TestHelpers.setOf(remote1, remote2), result.getLocalUnknownInserts());
    Assert.assertEquals(TestHelpers.setOf(local1, local2), result.getLocalUnknownDeletes());
    assertTrue(result.getRemoteDeletes().isEmpty());
  }
  ...
  public void resolveConflict_complex() {
    ...
    Assert.assertEquals(TestHelpers.setOf(remote3), result.getLocalContactInserts());
    Assert.assertEquals(TestHelpers.setOf(update(local2, merge2)), result.getLocalContactUpdates());
    Assert.assertEquals(TestHelpers.setOf(update(local4, merge4)), result.getLocalGroupV1Updates());
    Assert.assertEquals(TestHelpers.setOf(update(local5, merge5)), result.getLocalGroupV2Updates());
    Assert.assertEquals(TestHelpers.setOf(SignalStorageRecord.forContact(local3)), result.getRemoteInserts());
    Assert.assertEquals(TestHelpers.setOf(recordUpdate(remote1, merge1), recordUpdate(remote2, merge2), recordUpdate(remote4, merge4), recordUpdate(remote5, merge5)), result.getRemoteUpdates());
    assertEquals(Optional.of(update(local6, remote6)), result.getLocalAccountUpdate());
    Assert.assertEquals(TestHelpers.setOf(unknownRemote), result.getLocalUnknownInserts());
    Assert.assertEquals(TestHelpers.setOf(unknownLocal), result.getLocalUnknownDeletes());
    assertTrue(result.getRemoteDeletes().isEmpty());
  }
  ...
  public void createWriteOperation_generic() {
    ...
    StorageSyncHelper.WriteOperationResult result = StorageSyncHelper.createWriteOperation(1,
                                                                                           localKeys,
                                                                                           new MergeResult(TestHelpers.setOf(insert2),
                                                                                                           TestHelpers.setOf(update(old2, new2)),
                                                                                                           TestHelpers.setOf(insert3),
                                                                                                           TestHelpers.setOf(update(old3, new3)),
                                                                                                           TestHelpers.setOf(insert4),
                                                                                                           TestHelpers.setOf(update(old4, new4)),
                                                                                                           TestHelpers.setOf(unknownInsert),
                                                                                                           TestHelpers.setOf(unknownDelete),
                                                                                                           Optional.absent(),
                                                                                                           recordSetOf(insert1, insert3, insert4),
                                                                                                           TestHelpers.setOf(recordUpdate(old1, new1), recordUpdate(old3, new3), recordUpdate(old4, new4)),
                                                                                                           TestHelpers.setOf()));

    assertEquals(2, result.getManifest().getVersion());
    TestHelpers.assertContentsEqual(Arrays.asList(contactKey(3), contactKey(4), contactKey(5), contactKey(6), contactKey(7), contactKey(8), groupV1Key(9), groupV1Key(10), groupV2Key(19), groupV2Key(20), unknownKey(11)), result.getManifest().getStorageIds());
    assertEquals(recordSetOf(insert1, new1, insert3, new3, insert4, new4), new HashSet<>(result.getInserts()));
    TestHelpers.assertByteListEquals(TestHelpers.byteListOf(1, 100, 200), result.getDeletes());
  }
  ...
  private static SignalContactRecord.Builder contactBuilder(int key,
                                                            ...
                                                            String profileName)
  {
    return new SignalContactRecord.Builder(TestHelpers.byteArray(key), new SignalServiceAddress(uuid, e164))
                                  .setGivenName(profileName);
  }

  private static SignalAccountRecord account(int key) {
    return new SignalAccountRecord.Builder(TestHelpers.byteArray(key)).build();
  }
  ...
  {
    return new SignalGroupV1Record.Builder(TestHelpers.byteArray(key), TestHelpers.byteArray(groupId)).setBlocked(blocked).setProfileSharingEnabled(profileSharing).build();
  }
  ...
  {
    try {
      return new SignalGroupV2Record.Builder(TestHelpers.byteArray(key), new GroupMasterKey(TestHelpers.byteArray(groupId, 32))).setBlocked(blocked).setProfileSharingEnabled(profileSharing).build();
  ...
  private static SignalStorageRecord unknown(int key) {
    return SignalStorageRecord.forUnknown(StorageId.forType(TestHelpers.byteArray(key), UNKNOWN_TYPE));
  }

  private static List<StorageId> keyListOf(int... vals) {
    return Stream.of(TestHelpers.byteListOf(vals)).map(b -> StorageId.forType(b, 1)).toList();
  }

  private static StorageId contactKey(int val) {
    return StorageId.forContact(TestHelpers.byteArray(val));
  }

  private static StorageId groupV1Key(int val) {
    return StorageId.forGroupV1(TestHelpers.byteArray(val));
  }

  private static StorageId groupV2Key(int val) {
    return StorageId.forGroupV2(TestHelpers.byteArray(val));
  }

  private static StorageId unknownKey(int val) {
    return StorageId.forType(TestHelpers.byteArray(val), UNKNOWN_TYPE);
  }
  ...
  private static class TestGenerator implements StorageSyncHelper.KeyGenerator {
    ...
    public @NonNull byte[] generate() {
      return TestHelpers.byteArray(keys[index++]);
    }
  }
}
