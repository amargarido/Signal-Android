...
import org.eti.meta.Unbindable;
import org.eti.meta.components.AlertView;
import org.eti.meta.components.AvatarImageView;
import org.eti.meta.components.DeliveryStatusView;
import org.eti.meta.components.FromTextView;
import org.eti.meta.components.ThumbnailView;
import org.eti.meta.components.TypingIndicatorView;
import org.eti.meta.database.MmsSmsColumns;
...
import org.eti.meta.util.ExpirationUtil;
    ...
    BindableConversationListItem, Unbindable
{
  ...
  private static SpannableString getThreadDisplayBody(@NonNull Context context, @NonNull ThreadRecord thread) {
    if (thread.getGroupAddedBy() != null) {
      ...
      return emphasisAdded(context.getString(org.eti.meta.R.string.ThreadRecord_called));
    } else if (SmsDatabase.Types.isIncomingCall(thread.getType())) {
      return emphasisAdded(context.getString(org.eti.meta.R.string.ThreadRecord_called_you));
    } else if (SmsDatabase.Types.isMissedCall(thread.getType())) {
      return emphasisAdded(context.getString(org.eti.meta.R.string.ThreadRecord_missed_call));
