...
import br.eti.meta.database.documents.Document;
...
import br.eti.meta.database.model.databaseprotos.ReactionList;
import br.eti.meta.database.model.ReactionRecord;
import br.eti.meta.insights.InsightsConstants;
...
public abstract class MessagingDatabase extends Database implements MmsSmsColumns {
  ...
  final int getOutgoingSecureMessageCount(long threadId) {
    SQLiteDatabase db           = databaseHelper.getReadableDatabase();
    String[]       projection   = new String[] {"COUNT(*)"};
    String         query        = getOutgoingSecureMessageClause() + "AND " + THREAD_ID + " = ? AND" + "(" + getTypeField() + " & " + Types.GROUP_QUIT_BIT + " = 0)";
    String[]       args         = new String[]{String.valueOf(threadId)};
