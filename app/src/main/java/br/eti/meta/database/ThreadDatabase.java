...
import br.eti.meta.util.JsonUtils;
...
public class ThreadDatabase extends Database {
  ...
  public List<MessagingDatabase.MarkedMessageInfo> setAllThreadsRead() {
    ...
    final List<MessagingDatabase.MarkedMessageInfo> smsRecords = DatabaseFactory.getSmsDatabase(context).setAllMessagesRead();
    final List<MessagingDatabase.MarkedMessageInfo> mmsRecords = DatabaseFactory.getMmsDatabase(context).setAllMessagesRead();
  ...
  public List<MessagingDatabase.MarkedMessageInfo> setEntireThreadRead(long threadId) {
    setRead(threadId, false);

    final List<MessagingDatabase.MarkedMessageInfo> smsRecords = DatabaseFactory.getSmsDatabase(context).setEntireThreadRead(threadId);
    final List<MessagingDatabase.MarkedMessageInfo> mmsRecords = DatabaseFactory.getMmsDatabase(context).setEntireThreadRead(threadId);

    return Util.concatenatedList(smsRecords, mmsRecords);
  }

  public List<MessagingDatabase.MarkedMessageInfo> setRead(long threadId, boolean lastSeen) {
    return setRead(Collections.singletonList(threadId), lastSeen);
  }

  public List<MessagingDatabase.MarkedMessageInfo> setRead(Collection<Long> threadIds, boolean lastSeen) {
    SQLiteDatabase db = databaseHelper.getWritableDatabase();

    List<MessagingDatabase.MarkedMessageInfo> smsRecords = new LinkedList<>();
    List<MessagingDatabase.MarkedMessageInfo> mmsRecords = new LinkedList<>();
