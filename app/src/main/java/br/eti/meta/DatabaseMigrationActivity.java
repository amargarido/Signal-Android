...
import br.eti.meta.service.ApplicationMigrationService;
import br.eti.meta.database.SmsMigrator;
...
public class DatabaseMigrationActivity extends PassphraseRequiredActionBarActivity {
  ...
  private void handleStateProgress(SmsMigrator.ProgressDescription update) {
    this.promptLayout.setVisibility(View.GONE);
  ...
  private class ImportStateHandler extends Handler {
    ...
    public void handleMessage(Message message) {
      switch (message.what) {
      case ApplicationMigrationService.ImportState.STATE_IDLE:                   handleStateIdle();                                     break;
      case ApplicationMigrationService.ImportState.STATE_MIGRATING_IN_PROGRESS:  handleStateProgress((SmsMigrator.ProgressDescription)message.obj); break;
      case ApplicationMigrationService.ImportState.STATE_MIGRATING_COMPLETE:     handleImportComplete();                                break;
      }
    }
  }
  ...
  private class ImportServiceConnection implements ServiceConnection {
    ...
    public void onServiceConnected(ComponentName className, IBinder service) {
      ...
      ApplicationMigrationService.ImportState state = importService.getState();
