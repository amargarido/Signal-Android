...
import br.eti.meta.contacts.avatars.ResourceContactPhoto;
import br.eti.meta.dependencies.ApplicationDependencies;
import br.eti.meta.groups.GroupId;
import br.eti.meta.logging.Log;
import br.eti.meta.mediasend.AvatarSelectionActivity;
import br.eti.meta.mediasend.AvatarSelectionBottomSheetDialogFragment;
...
import br.eti.meta.registration.RegistrationUtil;
import br.eti.meta.util.CommunicationActions;
import br.eti.meta.util.FeatureFlags;
import br.eti.meta.util.StringUtil;
import br.eti.meta.util.concurrent.SimpleTask;
import br.eti.meta.util.text.AfterTextChanged;
import br.eti.meta.util.views.LearnMoreTextView;
...
public class EditProfileFragment extends Fragment {
  ...
  public static EditProfileFragment create(boolean excludeSystem,
    ...
    args.putBoolean(EditProfileActivity.EXCLUDE_SYSTEM, excludeSystem);
    args.putParcelable(EditProfileActivity.NEXT_INTENT, nextIntent);
    args.putBoolean(EditProfileActivity.DISPLAY_USERNAME, displayUsernameField);
    args.putInt(EditProfileActivity.NEXT_BUTTON_TEXT, nextButtonText);
  ...
  public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
    final GroupId      groupId     = GroupId.parseNullableOrThrow(requireArguments().getString(EditProfileActivity.GROUP_ID, null));
    final GroupId.Push pushGroupId = groupId != null ? groupId.requirePush() : null;

    initializeResources(view, pushGroupId != null);
    initializeViewModel(requireArguments().getBoolean(EditProfileActivity.EXCLUDE_SYSTEM, false), pushGroupId,  savedInstanceState != null);
  ...
  private void initializeResources(@NonNull View view, boolean isEditingGroup) {
    ...
    this.nextIntent         = arguments.getParcelable(EditProfileActivity.NEXT_INTENT);

    if (FeatureFlags.usernames() && arguments.getBoolean(EditProfileActivity.DISPLAY_USERNAME, false)) {
    ...
    this.finishButton.setText(arguments.getInt(EditProfileActivity.NEXT_BUTTON_TEXT, R.string.CreateProfileActivity_next));
    ...
    if (arguments.getBoolean(EditProfileActivity.SHOW_TOOLBAR, true)) {
