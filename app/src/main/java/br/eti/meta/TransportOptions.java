...
import br.eti.meta.util.MmsCharacterCalculator;
import br.eti.meta.util.PushCharacterCalculator;
import br.eti.meta.util.SmsCharacterCalculator;
import br.eti.meta.util.dualsim.SubscriptionInfoCompat;
import br.eti.meta.util.dualsim.SubscriptionManagerCompat;
...
public class TransportOptions {
  ...
  public @NonNull TransportOption getSelectedTransport() {
    ...
    if (defaultTransportType == TransportOption.Type.SMS) {
  ...
  public static @NonNull TransportOption getPushTransportOption(@NonNull Context context) {
    return new TransportOption(TransportOption.Type.TEXTSECURE,
  ...
  private @Nullable TransportOption findEnabledSmsTransportOption(Optional<Integer> subscriptionId) {
    if (subscriptionId.isPresent()) {
      ...
      for (TransportOption transportOption : enabledTransports) {
        if (transportOption.getType() == TransportOption.Type.SMS &&
                                                                        ...
                                                                        @NonNull CharacterCalculator characterCalculator)
  {
    ...
    if (subscriptions.size() < 2) {
      results.add(new TransportOption(TransportOption.Type.SMS, R.drawable.ic_send_unlock_24,
