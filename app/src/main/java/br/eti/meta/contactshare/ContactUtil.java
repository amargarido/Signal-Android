...
import br.eti.meta.util.SpanUtil;
...
public final class ContactUtil {
  ...
  public static @NonNull String getDisplayNumber(@NonNull Contact contact, @NonNull Locale locale) {
    Contact.Phone displayNumber = getPrimaryNumber(contact);
  ...
  Contact.Phone getPrimaryNumber(@NonNull Contact contact) {
    if (contact.getPhoneNumbers().size() == 0) {
      return null;
    }

    List<Contact.Phone> mobileNumbers = Stream.of(contact.getPhoneNumbers()).filter(number -> number.getType() == Contact.Phone.Type.MOBILE).toList();
  ...
  public static @NonNull String getPrettyPhoneNumber(@NonNull Contact.Phone phoneNumber, @NonNull Locale fallbackLocale) {
