...
public class NotificationChannels {
  ...
  public static synchronized @Nullable String createChannelFor(@NonNull Context context, @NonNull Recipient recipient) {
    ...
    RecipientDatabase.VibrateState vibrateState     = recipient.getMessageVibrate();
    boolean      vibrationEnabled = vibrateState == RecipientDatabase.VibrateState.DEFAULT ? TextSecurePreferences.isNotificationVibrateEnabled(context) : vibrateState == RecipientDatabase.VibrateState.ENABLED;
   ...
   */
  @WorkerThread
  public static synchronized void updateMessageVibrate(@NonNull Context context, @NonNull Recipient recipient, RecipientDatabase.VibrateState vibrateState) {
    if (!supported() || recipient.getNotificationChannel() == null) {
      return ;
    }
    Log.i(TAG, "Updating recipient vibrate with value: " + vibrateState);

    boolean enabled      = vibrateState == RecipientDatabase.VibrateState.DEFAULT ? getMessageVibrate(context) : vibrateState == RecipientDatabase.VibrateState.ENABLED;
    String  newChannelId = generateChannelIdFor(recipient);
    boolean success      = updateExistingChannel(ServiceUtil.getNotificationManager(context),
                                                 recipient.getNotificationChannel(),
                                                 newChannelId,
