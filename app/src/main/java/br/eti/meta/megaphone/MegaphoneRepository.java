...
public class MegaphoneRepository {
  ...
  public void onFirstEverAppLaunch() {
    executor.execute(() -> {
      database.markFinished(Megaphones.Event.REACTIONS);
      database.markFinished(Megaphones.Event.MESSAGE_REQUESTS);
      resetDatabaseCache();
    });
  }
  ...
  @AnyThread
  public void markSeen(@NonNull Megaphones.Event event) {
  ...
  private void init() {
    ...
    Set<Megaphones.Event>            events  = Stream.of(records).map(MegaphoneRecord::getEvent).collect(Collectors.toSet());
    Set<Megaphones.Event>            missing = Stream.of(Megaphones.Event.values()).filterNot(events::contains).collect(Collectors.toSet());
  ...
  @WorkerThread
  private @NonNull MegaphoneRecord getRecord(@NonNull Megaphones.Event event) {
