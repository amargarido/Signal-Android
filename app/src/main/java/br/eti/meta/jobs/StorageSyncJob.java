...
import br.eti.meta.database.StorageKeyDatabase;
...
import br.eti.meta.storage.StorageSyncModels;
import br.eti.meta.storage.StorageSyncValidations;
...
public class StorageSyncJob extends BaseJob {
  ...
  private boolean performSync() throws IOException, RetryLaterException, InvalidKeyException {
    ...
    if (remoteManifest.isPresent() && remoteManifestVersion > localManifestVersion) {
      ...
      StorageSyncHelper.KeyDifferenceResult keyDifference       = StorageSyncHelper.findKeyDifference(remoteManifest.get().getStorageIds(), allLocalStorageKeys);
      ...
      if (!keyDifference.isEmpty()) {
        ...
        StorageSyncHelper.MergeResult mergeResult          = StorageSyncHelper.resolveConflict(remoteOnly, localOnly);
        StorageSyncHelper.WriteOperationResult writeOperationResult = StorageSyncHelper.createWriteOperation(remoteManifest.get().getVersion(), allLocalStorageKeys, mergeResult);
    ...
    List<RecipientDatabase.RecipientSettings>       pendingUpdates       = recipientDatabase.getPendingRecipientSyncUpdates();
    List<RecipientDatabase.RecipientSettings>       pendingInsertions    = recipientDatabase.getPendingRecipientSyncInsertions();
    List<RecipientDatabase.RecipientSettings>       pendingDeletions     = recipientDatabase.getPendingRecipientSyncDeletions();
    ...
    Set<RecipientId>              archivedRecipients   = DatabaseFactory.getThreadDatabase(context).getArchivedRecipients();
    Optional<StorageSyncHelper.LocalWriteResult>    localWriteResult     = StorageSyncHelper.buildStorageUpdatesForLocal(localManifestVersion,
                                                                                                       allLocalStorageKeys,
                                                                                                       pendingUpdates,
                                                                                                       pendingInsertions,
                                                                                                       pendingDeletions,
    ...
    if (localWriteResult.isPresent()) {
      ...
      StorageSyncHelper.WriteOperationResult localWrite = localWriteResult.get().getWriteResult();
      ...
      clearIds.addAll(Stream.of(pendingUpdates).map(RecipientDatabase.RecipientSettings::getId).toList());
      clearIds.addAll(Stream.of(pendingInsertions).map(RecipientDatabase.RecipientSettings::getId).toList());
      clearIds.addAll(Stream.of(pendingDeletions).map(RecipientDatabase.RecipientSettings::getId).toList());
    ...
    for (StorageId id : ids) {
      switch (id.getType()) {
        ...
        case ManifestRecord.Identifier.Type.GROUPV2_VALUE:
          RecipientDatabase.RecipientSettings settings = recipientDatabase.getByStorageId(id.getRaw());
          if (settings != null) {
