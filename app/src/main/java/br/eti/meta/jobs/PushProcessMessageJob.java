...
import br.eti.meta.attachments.DatabaseAttachment;
import br.eti.meta.attachments.PointerAttachment;
import br.eti.meta.attachments.TombstoneAttachment;
import br.eti.meta.attachments.UriAttachment;
import br.eti.meta.contactshare.Contact;
import br.eti.meta.contactshare.ContactModelMapper;
import br.eti.meta.crypto.ProfileKeyUtil;
import br.eti.meta.crypto.SecurityEvent;
import br.eti.meta.crypto.storage.TextSecureSessionStore;
...
import br.eti.meta.database.GroupReceiptDatabase;
...
import br.eti.meta.database.SmsDatabase;
...
import br.eti.meta.groups.BadGroupIdException;
...
import br.eti.meta.groups.GroupV1MessageProcessor;
...
import br.eti.meta.linkpreview.Link;
import br.eti.meta.linkpreview.LinkPreview;
import br.eti.meta.linkpreview.LinkPreviewUtil;
import br.eti.meta.logging.Log;
import br.eti.meta.mms.IncomingMediaMessage;
...
import br.eti.meta.notifications.MessageNotifier;
import br.eti.meta.recipients.Recipient;
import br.eti.meta.recipients.RecipientId;
import br.eti.meta.ringrtc.IceCandidateParcel;
import br.eti.meta.ringrtc.RemotePeer;
import br.eti.meta.service.WebRtcCallService;
import br.eti.meta.sms.IncomingEncryptedMessage;
import br.eti.meta.sms.IncomingEndSessionMessage;
import br.eti.meta.sms.IncomingTextMessage;
...
public final class PushProcessMessageJob extends BaseJob {
  ...
  private PushProcessMessageJob(@NonNull Job.Parameters parameters,
  ...
  Job.Parameters createParameters(@Nullable SignalServiceContent content, @Nullable ExceptionMetadata exceptionMetadata) {
  ...
  private void handleSynchronizeReadMessage(@NonNull List<ReadMessage> readMessages, long envelopeTimestamp)
  {
    for (ReadMessage readMessage : readMessages) {
      List<Pair<Long, Long>> expiringText  = DatabaseFactory.getSmsDatabase(context).setTimestampRead(new MessagingDatabase.SyncMessageId(Recipient.externalPush(context, readMessage.getSender()).getId(), readMessage.getTimestamp()), envelopeTimestamp);
      List<Pair<Long, Long>> expiringMedia = DatabaseFactory.getMmsDatabase(context).setTimestampRead(new MessagingDatabase.SyncMessageId(Recipient.externalPush(context, readMessage.getSender()).getId(), readMessage.getTimestamp()), envelopeTimestamp);
      ...
      throws StorageFailedException, BadGroupIdException
  {
    ...
    Optional<MessagingDatabase.InsertResult> insertResult;
      ...
      throws MmsException, BadGroupIdException
  {
    ...
    try {
      ...
      if (recipients.isLocalNumber()) {
        MessagingDatabase.SyncMessageId id = new MessagingDatabase.SyncMessageId(recipients.getId(), message.getTimestamp());
  ...
  private void updateGroupReceiptStatus(@NonNull SentTranscriptMessage message, long messageId, @NonNull GroupId groupString) {
    ...
    Map<RecipientId, Integer> localReceipts     = Stream.of(receiptDatabase.getGroupReceiptInfo(messageId))
                                                        .collect(Collectors.toMap(GroupReceiptDatabase.GroupReceiptInfo::getRecipientId, GroupReceiptDatabase.GroupReceiptInfo::getStatus));
      ...
      throws MmsException, BadGroupIdException
  {
    ...
    if (recipient.isLocalNumber()) {
      MessagingDatabase.SyncMessageId id = new MessagingDatabase.SyncMessageId(recipient.getId(), message.getTimestamp());
      DatabaseFactory.getMmsSmsDatabase(context).incrementDeliveryReceiptCount(id, System.currentTimeMillis());
      DatabaseFactory.getMmsSmsDatabase(context).incrementReadReceiptCount(id, System.currentTimeMillis());
    }
                                           ...
                                           @NonNull Optional<Long> smsMessageId)
  {
    ...
    if (!smsMessageId.isPresent()) {
      Optional<MessagingDatabase.InsertResult> insertResult = insertPlaceholder(sender, senderDevice, timestamp);
                                            ...
                                            @NonNull Optional<Long> smsMessageId)
  {
    ...
    if (!smsMessageId.isPresent()) {
      Optional<MessagingDatabase.InsertResult> insertResult = insertPlaceholder(sender, senderDevice, timestamp, groupId);
                                    ...
                                    @NonNull Optional<Long> smsMessageId)
  {
    ...
    if (!smsMessageId.isPresent()) {
      Optional<MessagingDatabase.InsertResult> insertResult = insertPlaceholder(sender.getIdentifier(), senderDevice, timestamp, groupId);
...
//    }
                                     ...
                                     @NonNull SignalServiceReceiptMessage message)
  {
    for (long timestamp : message.getTimestamps()) {
      ...
      DatabaseFactory.getMmsSmsDatabase(context)
                     .incrementDeliveryReceiptCount(new MessagingDatabase.SyncMessageId(Recipient.externalPush(context, content.getSender()).getId(), timestamp), System.currentTimeMillis());
    }
  }
                                 ...
                                 @NonNull SignalServiceReceiptMessage message)
  {
    if (TextSecurePreferences.isReadReceiptsEnabled(context)) {
      for (long timestamp : message.getTimestamps()) {
        ...
        Recipient     sender  = Recipient.externalPush(context, content.getSender());
        MessagingDatabase.SyncMessageId id      = new MessagingDatabase.SyncMessageId(sender.getId(), timestamp);
  ...
  private Optional<MessagingDatabase.InsertResult> insertPlaceholder(@NonNull String sender, int senderDevice, long timestamp) {
    return insertPlaceholder(sender, senderDevice, timestamp, Optional.absent());
  }

  private Optional<MessagingDatabase.InsertResult> insertPlaceholder(@NonNull String sender, int senderDevice, long timestamp, Optional<GroupId> groupId) {
    SmsDatabase         database    = DatabaseFactory.getSmsDatabase(context);
  ...
  public static final class Factory implements Job.Factory<PushProcessMessageJob> {
    @Override
    public @NonNull PushProcessMessageJob create(@NonNull Job.Parameters parameters, @NonNull Data data) {
      try {
