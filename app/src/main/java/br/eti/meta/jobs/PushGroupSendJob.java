...
import br.eti.meta.jobmanager.JobLogger;
...
public class PushGroupSendJob extends PushSendJob {
  ...
  private @NonNull List<Recipient> getGroupMessageRecipients(@NonNull GroupId groupId, long messageId) {
    List<GroupReceiptDatabase.GroupReceiptInfo> destinations = DatabaseFactory.getGroupReceiptDatabase(context).getGroupReceiptInfo(messageId);

    if (!destinations.isEmpty()) {
      return Stream.of(destinations).map(GroupReceiptDatabase.GroupReceiptInfo::getRecipientId).map(Recipient::resolved).toList();
    }
