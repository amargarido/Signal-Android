...
public class ReactionsViewModel extends ViewModel {
  ...
  public @NonNull LiveData<List<ReactionsLoader.Reaction>> getRecipients() {
  ...
  public @NonNull LiveData<List<EmojiCount>> getEmojiCounts() {
    return Transformations.map(repository.getReactions(),
                               reactionList -> Stream.of(reactionList)
                                                     .groupBy(ReactionsLoader.Reaction::getEmoji)
  ...
  private int compareReactions(@NonNull Map.Entry<String, List<ReactionsLoader.Reaction>> lhs, @NonNull Map.Entry<String, List<ReactionsLoader.Reaction>> rhs) {
  ...
  private long getLatestTimestamp(List<ReactionsLoader.Reaction> reactions) {
    return Stream.of(reactions)
                 .max((a, b) -> Long.compare(a.getTimestamp(), b.getTimestamp()))
                 .map(ReactionsLoader.Reaction::getTimestamp)
                 .orElse(-1L);
  }

  interface Repository {
    LiveData<List<ReactionsLoader.Reaction>>  getReactions();
  }
