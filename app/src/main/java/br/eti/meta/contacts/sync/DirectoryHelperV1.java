...
import br.eti.meta.sms.IncomingJoinedMessage;
...
class DirectoryHelperV1 {
  ...
  @WorkerThread
  static RecipientDatabase.RegisteredState refreshDirectoryFor(@NonNull Context context, @NonNull Recipient recipient, boolean notifyOfNewUsers) throws IOException {
    RecipientDatabase recipientDatabase = DatabaseFactory.getRecipientDatabase(context);
    ...
    if (recipient.getUuid().isPresent() && !recipient.getE164().isPresent()) {
      ...
      return isRegistered ? RecipientDatabase.RegisteredState.REGISTERED : RecipientDatabase.RegisteredState.NOT_REGISTERED;
    }
                                     ...
                                     @NonNull  List<RecipientId> newUsers)
  {
    ...
    for (RecipientId newUser: newUsers) {
      ...
      if (!SessionUtil.hasSession(context, recipient.getId()) && !recipient.isLocalNumber()) {
        ...
        Optional<MessagingDatabase.InsertResult> insertResult = DatabaseFactory.getSmsDatabase(context).insertMessageInbox(message);
  ...
  private static Optional<AccountHolder> getOrCreateAccount(Context context) {
    ...
    Account[]      accounts       = accountManager.getAccountsByType("br.eti.meta");
  ...
  private static Optional<AccountHolder> createAccount(Context context) {
    AccountManager accountManager = AccountManager.get(context);
    Account        account        = new Account(context.getString(R.string.app_name), "br.eti.meta");
  ...
  private static RecipientDatabase.RegisteredState getRegisteredState(@NonNull Context                     context,
                                                                      @NonNull SignalServiceAccountManager accountManager,
                                                                      @NonNull RecipientDatabase           recipientDatabase,
                                                                      @NonNull Recipient                   recipient)
      throws IOException
  {
    boolean                       activeUser    = recipient.resolve().getRegistered() == RecipientDatabase.RegisteredState.REGISTERED;
