...
import br.eti.meta.BindableConversationListItem;
import br.eti.meta.R;
import br.eti.meta.Unbindable;
import br.eti.meta.components.AlertView;
import br.eti.meta.components.AvatarImageView;
import br.eti.meta.components.DeliveryStatusView;
import br.eti.meta.components.FromTextView;
import br.eti.meta.components.ThumbnailView;
import br.eti.meta.components.TypingIndicatorView;
...
import br.eti.meta.util.ExpirationUtil;
        ...
        BindableConversationListItem, Unbindable
{
  ...
  private static SpannableString getThreadDisplayBody(@NonNull Context context, @NonNull ThreadRecord thread) {
    if (thread.getGroupAddedBy() != null) {
      ...
      return emphasisAdded(context.getString(br.eti.meta.R.string.ThreadRecord_called));
    } else if (SmsDatabase.Types.isIncomingCall(thread.getType())) {
      return emphasisAdded(context.getString(br.eti.meta.R.string.ThreadRecord_called_you));
    } else if (SmsDatabase.Types.isMissedCall(thread.getType())) {
      return emphasisAdded(context.getString(br.eti.meta.R.string.ThreadRecord_missed_call));
